...
def send_df_to_dashboard(df, id):
    tag = [str(t.emoji) for t in df.select("emoji").take(10)]
    url = 'http://localhost:8050/update_data'
    request_data = {'value' : str(tag)} #request_data = {'tag': str(tag), 'tag_count': str(tag_count)}
    print('update dashboard')
    response = requests.post(url, data=request_data)

new.writeStream.outputMode("append").foreachBatch(send_df_to_dashboard).trigger(processingTime="3 seconds").start()
