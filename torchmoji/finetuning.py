...
def finetune(model, texts, labels, nb_classes, batch_size, method,
    ...
    if method == 'last':
        ...
        #adam = optim.Adam((p for p in model.parameters()), lr=lr)
...
def evaluate_using_acc(model, test_gen):
    ...
    for i, data in enumerate(test_gen):
        ...
        if model.nb_classes > 2:
            ...
            #pred = (outs >= 0).long().squeeze()
            pred = (outs >= 0).long()
            #print(pred, y)
            #print((pred == y).double().sum())
            #print(len(pred))
...
def fit_model(model, loss_op, optim_op, train_gen, val_gen, epochs,
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(train_gen):
            ...
            #X_train = Variable(X_train.float(), requires_grad=True)
            #y_train = Variable(y_train.float(), requires_grad=True)
            #X_train = X_train.type(dtype=torch.float)
            #y_train = y_train.type(dtype=torch.float)
            ...
            #loss = Variable(calc_loss(loss_op, output, y_train), requires_grad=True)
            loss = calc_loss(loss_op, output, y_train).requires_grad_(True)
